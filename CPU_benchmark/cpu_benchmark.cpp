...
void benchmark_gemm(int M, int N, int K, int num_iterations = 10) {
    ...
    double cpu_opt_time = std::chrono::duration<double, std::milli>(end1 - start1).count() / num_iterations;
    ...
    std::cout << "Correctness - CPU OPT vs CPU Naive: " << (cpu_opt_match ? "PASS" : "FAIL") << std::endl;
