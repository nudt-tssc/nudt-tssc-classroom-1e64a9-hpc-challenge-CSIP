...
void benchmark_gemm(int M, int N, int K, int num_iterations = 10) {
    ...
    std::vector<float> h_C_gpu_opt(M * N, 0.0f);
    ...
    for (int i = 0; i < num_iterations; i++) {
        CUDA_CHECK(cudaMemset(d_C, 0, M * N * sizeof(float)));
        gpu_gemm_opt(d_A, d_B, d_C, M, N, K, alpha, beta);
    }
    end = std::chrono::high_resolution_clock::now();
    double gpu_opt_time = std::chrono::duration<double, std::milli>(end - start).count() / num_iterations;
    double gpu_opt_gflops = calculate_gflops(M, N, K, gpu_opt_time);
    
    // Copy result back for correctness check
    CUDA_CHECK(cudaMemcpy(h_C_gpu_opt.data(), d_C, M * N * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    std::cout << "GPU OPT:        " << std::setw(8) << gpu_opt_time << " ms, " 
              << std::setw(8) << gpu_opt_gflops << " GFLOPS (Speedup: " 
              << cpu_time / gpu_opt_time << "x)" << std::endl;
